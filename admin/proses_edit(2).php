...
if($simpan=="user"){
    ...
    $status = $_POST['status'];
        ...
        if(in_array($ekstensi, $ekstensi_diperbolehkan)){
            if(!$ukuran < 700000){
                ...
                $target = $server."/admin/daftar/barang/".$gambar_old;
                unlink($target);
    ...
    if($_POST['simpan']){
        ...
        $sumber = $_FILES['gambar']['tmp_name'];
        $server = $_SERVER['DOCUMENT_ROOT'];
        $folder = $server."/admin/daftar/artikel/";
                ...
                if(!$ukuran < 700000){
                    ...
                    if(empty($judul) || empty($isi_a) || empty($penulis) || empty($tahun) || empty($gambar) ){
                        header("location: ".BASE_URL."index.php?page=admin/admin&admin=edit&tabel=artikel&notif=requiere&$dataform");
                                ...
                                $target = $server."/admin/daftar/artikel/".$gambar_old;
                                unlink($target);
        ...
        $folder = 'daftar/rs/';
            ...
            if(in_array($ekstensi, $ekstensi_diperbolehkan)){
                if(!$ukuran < 700000){
                    ...
                    $dataform = http_build_query($_POST);
                    $server = $_SERVER['DOCUMENT_ROOT'];
                    $target = $server."/admin/daftar/rs/".$gambar_old;
                    unlink($target);
